...
classdef BLR < Parameter_Estimator
    ...
    methods
        function obj = BLR(sys, prior_mean, prior_variance, data, features)
            ...
            % Lookup variables used in the following code for readability
            noiseCovariance = obj.params.noiseCovariance;
            
            if ~isempty(data.x) && ~isempty(data.y)
                Sigma_inv = 1/noiseCovariance*obj.features(data.x)*obj.features(data.x)'+inv(obj.Sigma);
                
                obj.theta = Sigma_inv^-1*(obj.Sigma^-1*obj.theta + 1/noiseCovariance*obj.features(data.x)*data.y');
